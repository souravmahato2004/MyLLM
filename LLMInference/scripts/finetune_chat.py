"""
Phase 5 — instruction fine-tune GPT-2 124M into a chat model.

Starts from OpenAI's pretrained weights (via load_pretrained_gpt2_124m) and
fine-tunes on the instruction dataset so the model learns to *respond* to an
...
DATA_PATH = PROJECT_ROOT / "data" / "instruction" / "instruction-data.json"
CHECKPOINT_PATH = PROJECT_ROOT / "data" / "checkpoints" / "gpt2_124m_chat.pt"

# --- Knobs -----------------------------------------------------------------
MAX_ENTRIES = None     # <-- smoke run. Set to None for the full ~1100-entry run.
NUM_EPOCHS = 2
...
def main() -> None:
    ...
    print("Loading pretrained GPT-2 124M as the starting point...")
